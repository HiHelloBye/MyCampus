            ...
            <div class="container"> 
                <p>
                    <form action="bookSearch.do" method="POST" name="frm">
                        <input style="float:left; width:90%;" type="text" name="search" placeholder="검색하고 싶은 책 이름을 검색하세요." autocomplete="off" />
        ...
        <section id="features">
            <div class="container" >
                <div class="row aln-center" style="width: 80%; margin: auto;">
                    ...
                    <table>
                        <c:forEach var="book" items="${booksellList}">
                        	<c:choose>
            ...
            <c:if test="${pg.startPage > pg.pageBlock }">
		        <a href="booksellList.do?currentPage=${pg.startPage-pg.pageBlock}">[다음]</a>
		    </c:if>
		    <c:forEach var="i" begin="${pg.startPage }" end="${pg.endPage }">
		       <a href="booksellList.do?currentPage=${i}">[${i}]</a>
		    </c:forEach>
		    <c:if test="${pg.endPage < pg.totalPage }">
		      <a href="booksellList.do?currentPage=${pg.startPage+pg.pageBlock}">[이전]</a>
		    </c:if> 
        </section>  
    </div>
    <!-- Scripts -->
