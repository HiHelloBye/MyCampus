...
<title>스터디 게시판</title>
<link href="../cssStudy/assets/css/main.css?ver=2" rel="stylesheet" type="text/css">
...
<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
	<c:set var="study_title" value="${study.study_title }" />
	<%
		String study_no = request.getParameter("study_no");		
		String study_title = pageContext.getAttribute("study_title").toString();
		System.out.println("study_title->" + study_title);
				...
				<div class="row">
					<div id="content" class="col-8 col-12-medium">
						<article class="box post">
							<header id="myPro">
								<font class="font_title">${study.study_title }</font>
								<br />
							...
							<c:if test="${sessionScope.member_nick != study.study_writer }">
								<a onclick="return confirm('이 글을 정말로 신고하시겠습니까?')"
									class="button small"
									href="../insertWarning.do?type_no=1&study_leceval=${study.study_no }&study_no=${study.study_no}&member_no=${sessionScope.member_no }&warn_state=1">신고</a>
							</c:if>
						</article>
					</div>
				...
				<table>
					<tr>
						...
						<td style="width: 580px;">
							...
							name="comm_writer" value="${sessionScope.member_nick }"> 
							<textarea id="input1" name="comm_cont" required="required"
								rows="1" cols="80" style="resize: none;"></textarea> <br>
							<div>
								...
								<a style="size: 1; width: 50px; float: right;" href="javascript:popupOpen();"
									class="button small">
									<img src="../resources/bs/1.png" style="text-align:left; width: 100%; padding-top: 0.5em;">
								</a>	
								<button style="float: right;" type="button" id="button1" onclick="writeCmt()">댓글<br>쓰기</button>
					...
					<c:choose> 
						<c:when test="${studyComm.comm_type==0 && (sessionScope.member_nick == '관리자' || sessionScope.member_nick ==studyComm.comm_writer || sessionScope.member_nick == study.study_writer)}">
						
							<tr id="list9">
								<td style=width:100px;><p>
										<img src="${studyComm.member_photo}"><br>${studyComm.comm_writer }</p></td>
								<td><c:if test="${studyComm.bstep > 0}">
										<img src='../resources/emoticon/arrow.png' alt="" width="15"
											class="arrow">
									</c:if>${studyComm.comm_cont }
									</c:when>
									<c:when test="${studyComm.comm_type==1 }">
									<tr id="list9">
										<td><p>
												<img src="${studyComm.member_photo}"><br>${studyComm.comm_writer }</p></td>
										<td><c:if test="${studyComm.bstep > 0}">
									...
									<c:when test="${studyComm.comm_type==3 }">
									<tr id="list9">
										<td></td>
										<td><p>삭제된 댓글입니다</p>
									</c:when>
								...
								<td>비밀</td>
								<td><c:if test="${studyComm.bstep > 0}">
										<img src='../resources/emoticon/arrow.png' alt="" width="15" class="arrow">
									</c:if>비밀 댓글입니다
									</c:otherwise>
									...
									<div id="hClose01${studyComm.comm_no }" style="display: none;">
										...
										<textarea name="comm_cont" id="input2${studyComm.comm_no }"
											rows="1" cols="80" style="resize: none;">${studyComm.comm_cont }</textarea>
										<br> <input type="radio" name="comm_type2" value="1"
											<c:if test="${studyComm.comm_type == 1}">checked="checked"</c:if>
											required="required" id="radioid${studyComm.comm_no}"> <label for="radioid${studyComm.comm_no}">공개</label>
										<input type="radio" name="comm_type2" value="0"
											required="required"
											<c:if test="${studyComm.comm_type == 0}">checked="checked"</c:if>
											id="radioid${studyComm.comm_no}"> <label for="radioid${studyComm.comm_no}">비공개</label>
									...
									<div id="hClose02${studyComm.comm_no }" style="display: none;">									
										<textarea name="comm_cont" id="input3${studyComm.comm_no }"
											rows="1" cols="80" style="resize: none;"></textarea>
										<br>
										<div>
										<input type="radio" name="comm_type3" value="1"	id="radio${studyComm.comm_no}" /> <label for="radio${studyComm.comm_no}">공개</label>
										<input type="radio" name="comm_type3" value="0" id="radio1${studyComm.comm_no}" /> <label for="radio1${studyComm.comm_no}">비공개</label>
										</div>
										<div style="float: right;">
											<div style="float: right">
								...
								<td><input type="hidden" id="comm_cont${studyComm.comm_no }"
									name="comm_cont" value="${studyComm.comm_cont }">
									<input type="hidden" name="bgroup" id="bgroup${studyComm.comm_no}"
									value="${studyComm.bgroup}">
									<input type="hidden" name="bstep" id="bstep${studyComm.comm_no}"
									...
									<c:if test="${'관리자'==sessionScope.member_nick}">
										...
										test="${sessionScope.member_nick==studyComm.comm_writer && studyComm.comm_type!=3}">
						...
						paragraph_toggle(document.getElementById('hClose01${studyComm.comm_no }')); return false;"
									...
									<c:if test="${sessionScope.member_nick!=null && studyComm.comm_type!=3}">
