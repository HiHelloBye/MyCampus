...
<script type="text/javascript">
	function itemChange() {
		...
		for (var count = 0; count < changeItem.length; count++) {
			if (selectItem == "어학") {
				var option = $("<option value="+f[count]+">"
						+ changeItem[count] + "</option>");
				$('#select2').append(option);
			} else {
				var option = $("<option value="+k[count]+">"
						+ changeItem[count] + "</option>");
				$('#select2').append(option);
			}
		}
...
<script type="text/javascript">
	$(function() {
		...
		nhn.husky.EZCreator.createInIFrame({
			...
			fOnAppLoad : function() {
				editor_object.getById["smarteditor"].exec("PASTE_HTML",
						[ content ])
			},
...
form input[type="text"], form input[type="email"]
	input[type="password"], form select, form textarea {
			...
			<div class="row">
				<div id="content" class="col-8 col-12-medium">
					<form action="studyUpdate.do" method="post">
						...
						<input type="hidden" name="study_warn"
							value="${study.study_warn }"> <input type="hidden"
							name="study_state" value="${study.study_state }">
						<table>

							<tr>
								<th style="width: 100px;">제목 :</th>
								<td><input type="text" name="study_title"
									value="${study.study_title }" maxlength="50"
									required="required"></td>
							</tr>
							<tr>
								<th>종류 :</th>
								<td><input type="hidden" name="study_writer"
									value="${sessionScope.member_nick }"> <select
							...
							<tr>
								...
								<td>
									<!-- <textarea style="resize: none;" name="study_cont"rows="4" cols="50"required="required"></textarea> -->
									<textarea name="study_cont" id="smarteditor" rows="10"
										cols="100" style="width: 766px; height: 412px;"
										required="required">${study.study_cont }</textarea>
								</td>
							</tr>
